...
(define (accept? current-state state-transition-table)
    (cdr (nth (state-transition-table) (+ 1 current-state))))
...
(define (dfa current-state input state-transition-table)
  (let ((next-state (get-next-state state-transition-table current-state (car input))))
    (if (null? next-state)
      #f
      (if (null? (cdr input))
        (accept? next-state state-transition-table)
...
(define (state-transition-table-of-real-number)
  (list       (list zero one-to-nine dot)
        ...
        (cons (list 1    1           2  ) #t)
        (cons (list 2    2           () ) #t)))
(define (tokenize char-list)
  (let ((result (list)))
    (define (recurse char-list result)
      (print char-list)
      (recurse (cdr char-list) result))
    (recurse char-list result)))
